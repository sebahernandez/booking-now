...
async function main() {
  ...
  if (!photoSession) {
    photoSession = await prisma.service.create({
      data: {
        ...
        description: 'Professional photography for portraits, events, or commercial use',
        duration: 60,
        price: 150,
      },
    });
  ...
  for (const link of serviceLinks) {
    ...
    if (!existing) {
      ...
      console.log(`✅ Linked professional ${link.professionalId} to service ${link.serviceId}`)
    } else {
  ...
  for (const professional of [john, sarah, mike]) {
    for (const dayOfWeek of workDays) {
      ...
      if (!existing) {
        ...
        console.log(`✅ Created availability slot for professional ${professional.id}, day ${dayOfWeek}`)
      } else {
        console.log(`ℹ️ Availability slot already exists for professional ${professional.id}, day ${dayOfWeek}`)
      }
    }
  }
