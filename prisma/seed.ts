...
async function main() {
  ...
  // Custom user and professional (generated by Copilot)
  let copilotUser = await prisma.user.findUnique({
    where: { email: "copilot.pro@bookingnow.com" },
  });
  if (!copilotUser) {
    copilotUser = await prisma.user.create({
      data: {
        email: "copilot.pro@bookingnow.com",
        name: "Copilot Pro",
        phone: "+56987654321",
        role: UserRole.PROFESSIONAL,
        tenantId: defaultTenant.id,
    ...
    console.log("✅ Created Copilot Pro user");
  } else {
    console.log("ℹ️ Copilot Pro user already exists");
  }
  ...
  // Custom professional profile (generated by Copilot)
  let copilotPro = await prisma.professional.findUnique({
    where: { userId: copilotUser.id },
  });
  if (!copilotPro) {
    copilotPro = await prisma.professional.create({
      data: {
        userId: copilotUser.id,
        tenantId: defaultTenant.id,
        bio: "Fotógrafo IA especializado en automatización y retratos creativos.",
        hourlyRate: 180,
      },
    });
    console.log("✅ Created Copilot Pro professional profile");
  } else {
    console.log("ℹ️ Copilot Pro professional profile already exists");
  }
